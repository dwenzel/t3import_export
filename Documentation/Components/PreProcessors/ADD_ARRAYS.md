...
Only the target field will be changed. If the content of a field is not an array
it will be ignored.

## Configuration
## required
* **class** fully qualified class name, i.e.
  *CPSIT\T3importExport\Component\PreProcessor\AddArrays*
* **config.targetField** *string* field name of record to which the content of
the fields are added. (Should be an array too)
...
* **config.disable** *boolean*
    ...
    If the value is an array and can be interpreted as content object it will
    be rendered. The result of the rendering
